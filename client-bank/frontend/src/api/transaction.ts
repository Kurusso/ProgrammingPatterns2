...
export class TransactionService {
    static async performTransaction(accountId: string, money: number, currency: Currency, transactionType: TransactionType, userId: string) {
        try {
            ...
            let request_url = `${(transactionType === TransactionType.Deposit ? magicConsts.depositEndpoint : magicConsts.withdrawEndpoint)}?accountId=${accountId}&userId=${userId}&money=${money}&currency=${currency}`;
